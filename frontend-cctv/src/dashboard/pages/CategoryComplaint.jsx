...
export default function ComplaintCategoryManagement() {
  ...
  // FULL CRUD — konsisten dengan role yang udah dikonfirmasi di backend.
  // Petugas GSL read-only di sini (cuma full CRUD di Komplain GSL sendiri).
  const canManage = ['Admin', 'Petugas CCTV', 'Manager HSE', 'Petugas HSE'].includes(role);
  ...
  // --- Fetch Complaint Data (buat chart & ranking) ---
  const fetchComplaints = async () => {
    try {
      const token = localStorage.getItem('token');

      if (!token) {
        console.error('Token tidak ditemukan untuk komplain.');
        return;
      }
      ...
      console.error('Error fetching complaints for chart:', error);
  ...
  const getCategoryIcon = (category) => {
    ...
    if (name.includes('kehilangan')) return <FaBoxOpen />;
    if (name.includes('pencurian')) return <FaUserSecret />;
    if (name.includes('kerusakan kendaraan')) return <FaCarSide />;
    if (name.includes('lain')) return <FaClipboardList />;
  ...
  }, [kategoriList, complaintList]);

  // Komplain yang kategorinya sudah dihapus/diganti nama (tidak match ke
  // kategoriList aktif manapun) dihitung terpisah, bukan diam-diam di-skip
  // dari ranking di atas — sama seperti fix pola serupa di Data Lokasi &
  // Komplain GSL.
  const unmatchedCategoryCount = useMemo(() => {
    const activeCategoryNames = new Set(kategoriList.map((cat) => cat.name));

    return complaintList.filter((c) => !c.category?.name || !activeCategoryNames.has(c.category.name))
      .length;
  }, [kategoriList, complaintList]);
  ...
  // konsisten dan bisa menjumlah ke 100% (bukan complaintList.length mentah
  // yang ikut menghitung komplain yang sudah tidak tertaut kategori).
  const totalMatchedComplaints = categoryStats.reduce((sum, item) => sum + item.total, 0);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Swal.fire({
        ...
        text:
          err.response?.data?.message || err.response?.data || err.message || 'Terjadi kesalahan',
  ...
  const handleDelete = async (id) => {
    const result = await Swal.fire({
      ...
      text: 'Kategori yang masih dipakai di data komplain tidak bisa dihapus.',
      icon: 'warning',
      showCancelButton: true,
  ...
  return (
    <Layout>
      ...
      <section className="p-4 w-full max-w-full mx-auto flex flex-col lg:flex-row gap-6">
        {/* CHART */}
        <div className="w-full lg:w-3/4 h-[380px] sm:h-[420px] lg:h-[450px] bg-white/20 backdrop-blur-2xl border border-white/30 rounded-3xl shadow-2xl p-6">
          <div className="flex justify-between items-center mb-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-gray-800">Grafik Komplain per Kategori</h2>
                <p className="text-gray-500 text-sm">
                  Distribusi jumlah komplain berdasarkan kategori
                </p>
              </div>
            </div>
          ...
          {unmatchedCategoryCount > 0 && (
            <div className="mb-4 flex items-start gap-2 rounded-xl bg-amber-50 border border-amber-200 px-4 py-2.5 text-amber-800">
              ...
              <p className="text-xs sm:text-sm leading-snug">
                <b>{unmatchedCategoryCount}</b> komplain tidak tertaut ke kategori aktif mana pun
          ...
          <div style={{ height: 'calc(100% - 70px)', width: '100%' }}>
            {kategoriList.length > 0 && complaintList.length > 0 ? (
              <Pie
                data={{
              ...
              <div className="h-full flex items-center justify-center">
                <p className="text-gray-500">Belum ada data kategori atau komplain.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="w-full lg:w-[320px] h-[380px] sm:h-[420px] lg:h-[450px] bg-white/80 backdrop-blur-xl rounded-3xl p-5 shadow-2xl border border-white/30 overflow-y-auto">
          ...
          <div className="space-y-4">
            {categoryStats.length > 0 ? (
              categoryStats.map((item, index) => {
                ...
                const maxTotal = Math.max(...categoryStats.map((x) => x.total), 1);
                const percentage = ((item.total / maxTotal) * 100).toFixed(0);
                  ...
                  <div
                    ...
                    onClick={() => {
                      setSelectedCategory({
                        name: item.name,
                        total: item.total,
                        percentage: totalMatchedComplaints
                          ? ((item.total / totalMatchedComplaints) * 100).toFixed(1)
                          : '0',
                        rank: index + 1,
                        color,
                      });
                    }}
                    className="group cursor-pointer bg-white rounded-2xl p-4 shadow-md border border-gray-100 hover:-translate-y-1 hover:shadow-xl transition-all duration-300"
                  >
                    <div className="grid grid-cols-[auto_1fr_auto] items-center gap-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <p className="font-semibold text-gray-800 text-sm leading-tight line-clamp-3">
                            {item.name}
                          </p>
                          <p className="text-xs text-gray-500 mt-1">Total Komplain</p>
                        </div>
                      </div>

                      <div className="text-center">
                        <h2 className="text-xl font-bold text-gray-900">{item.total}</h2>
        ...
        {selectedCategory && (
          <div
            className="fixed inset-0 z-50 bg-black/40 backdrop-blur-md flex items-center justify-center p-2 sm:p-4 md:p-6"
            onClick={() => setSelectedCategory(null)}
          >
            <div
              className="w-full max-w-xl bg-white rounded-2xl md:rounded-3xl shadow-2xl max-h-[95vh] overflow-y-auto p-4 sm:p-6 md:p-8 animate-in fade-in zoom-in duration-300"
            ...
            >
              <div className="flex justify-between items-start">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-500 text-sm">Detail Statistik Kategori</p>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-3 gap-4 mt-8">
                <div className="bg-blue-50 rounded-2xl p-5">
                  ...
                  <h3 className="text-4xl font-bold text-blue-600">#{selectedCategory.rank}</h3>
                </div>

                <div className="bg-red-50 rounded-2xl p-5">
                  <p className="text-gray-500 text-sm">Total</p>
                  <h3 className="text-4xl font-bold text-red-600">{selectedCategory.total}</h3>
                </div>
              ...
              >
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <h3 className="font-bold">Insight Kategori</h3>
                </div>
                ...
                <p className="text-gray-600 leading-relaxed">
                  ...
                  dari seluruh kategori komplain.
                </p>
              </div>
            </div>
          </div>
      ...
      <section className="p-3 md:p-6">
        ...
        >
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[11px] leading-relaxed text-gray-400 mt-1 truncate max-w-[150px]">
                Kelola kategori klasifikasi komplain GSL
              </p>
            </div>
          </div>
          ...
          {canManage && (
            <button
              ...
              className={`group flex items-center justify-center gap-2 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 text-white text-xs font-semibold shadow-[0_10px_25px_rgba(37,99,235,.35)] hover:scale-105 transition-all duration-300 shrink-0 ${showAddKategoriBtnText ? 'px-5 py-3 rounded-2xl' : 'w-11 h-11'}`}
            >
        ...
        >
          ...
          <button
            ...
            title="Buka Kalender"
            className={`group flex items-center justify-center gap-2 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 text-white text-xs font-semibold shadow-[0_10px_25px_rgba(37,99,235,.35)] hover:scale-105 transition-all duration-300 shrink-0 ${showKategoriCalendarText ? 'px-5 py-3 rounded-2xl' : 'w-11 h-11'}`}
          >
        ...
        {showForm && (
          ...
          >
            <div
              onClick={(e) => e.stopPropagation()}
              className="bg-white/90 backdrop-blur-3xl rounded-2xl md:rounded-[40px] shadow-[0_40px_100px_rgba(0,0,0,.2)] w-full max-w-xl max-h-[95vh] overflow-y-auto p-4 sm:p-6 md:p-10"
            >
              <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-8">
                <div>
                  ...
                  <h2 className="flex items-center gap-2 text-2xl md:text-3xl font-bold text-gray-800 mt-3">
                    {isEditing ? (
                      <>
                  ...
                  <p className="text-gray-500 mt-2">Kelola kategori klasifikasi komplain</p>
                </div>
                ...
                <button
                  ...
                  className="w-12 h-12 flex items-center justify-center rounded-full bg-gray-100 hover:bg-red-500 hover:text-white duration-300 shrink-0"
                >
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                  ...
                  <input
                    ...
                    className="w-full mt-3 px-5 py-3 rounded-2xl border border-gray-300 focus:ring-2 focus:ring-blue-500 outline-none transition"
                  />
                </div>
                      ...
                      {canManage && (
                        <div className="flex justify-center gap-2">
                          <button
                            ...
                            title="Ubah Data"
                            className="bg-yellow-400 hover:bg-yellow-500 text-white p-2 rounded-lg transition shadow-sm hover:scale-105 focus:outline-none focus:ring-2 focus:ring-yellow-300 flex items-center justify-center"
                          >
                            <FiEdit2 size={17} />
                          </button>
                          ...
                          <button
                            ...
                            title="Hapus Data"
                            className="bg-red-500 hover:bg-red-600 text-white p-2 rounded-lg transition shadow-sm hover:scale-105 focus:outline-none focus:ring-2 focus:ring-red-300 flex items-center justify-center"
                          >
                            <FiTrash2 size={17} />
                          </button>
                        </div>

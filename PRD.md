...
Pondok Indah Mall — **Versi Draft 1.7** — Diperbarui 12 September 2026 — HSE
...
- **⚠️ Baru, dari audit kode — ditemukan dan diperbaiki di Draft 1.7:** Audit gejala yang sama diperluas ke sisa halaman `frontend-cctv` yang belum dicek. Tiga bug baru ditemukan (kembali ke mekanisme Flavor A — reference yang tidak match hilang diam-diam, bukan filter-scope):
  ...
  - **`User.jsx` (dilaporkan, BELUM diperbaiki — perlu keputusan produk):** role distribution memakai 6 role tetap yang match dropdown saat ini, tapi kolom `Role` di database bertipe bebas (`varchar`, tanpa enum constraint di level DB/model) — user dengan role di luar daftar berpotensi hilang dari bar chart tanpa warning. Belum dikonfirmasi ada data seperti itu di database aktual (kemungkinan cuma risiko laten, bukan bug yang sudah termanifestasi) — didiskusikan dulu dengan pemilik produk sebelum diubah. Ditambahkan ke Open Questions #8.
  - **`CameraOccupancy.jsx` (dilaporkan, BELUM diperbaiki — ambiguitas desain, bukan bug pasti):** panel "Occupancy Summary" menghitung persentase IP/Analog pakai `total_kamera` (field input manual) sebagai penyebut, sementara panel "Camera Type By Area" menghitung metrik serupa pakai `ip + analog` (hasil hitung) sebagai penyebut. Kedua angka bisa beda kalau `total_kamera` tidak selalu sama dengan `ip + analog` di data — tapi ini mungkin memang disengaja (`total_kamera` = kapasitas terpasang vs `ip + analog` = realisasi saat ini), bukan silent-drop seperti kasus lain. Ditambahkan ke Open Questions #8.
...
8. **Dua temuan audit Draft 1.7 yang sengaja belum diubah, butuh keputusan produk:**
   - Apakah field `Role` di tabel `User` sebaiknya dibatasi ke enum tetap (bukan `varchar` bebas), supaya role distribution di `User.jsx` tidak berisiko diam-diam kehilangan user dengan role di luar daftar yang di-hardcode di frontend?
   - Di `CameraOccupancy.jsx`, apakah `total_kamera` (input manual) memang dimaksudkan sebagai "kapasitas terpasang" yang bisa beda dari `ip + analog` (realisasi saat ini), atau seharusnya kedua panel occupancy di halaman itu selalu memakai penyebut yang sama? Kalau memang dua konsep yang berbeda, sebaiknya label di UI diperjelas supaya tidak terbaca sebagai dua sumber angka yang saling bertentangan.
